...
import { USER_REGISTER_REQUEST, USER_REGISTER_FAILURE, USER_LOGIN_REQUEST, USER_LOGIN_FAILURE, USER_FORGOT_PASSWORD, FORGOT_PASSWORD_FAILURE, USER_RESET_PASSWORD, RESET_PASSWORD_FAILURE, USER_CHANGE_PASSWORD, CHANGE_PASSWORD_FAILURE, VERIFY_USER_EMAIL, VERIFY_USER_EMAIL_FAILURE, USER_PROFILE_REQUEST, USER_PROFILE_FAILURE, GET_AUTH_USER_REQUEST, GET_AUTH_USER_FAILURE } from "../constants/constants";
...
export const updateUser = (userInfo) => async (dispatch) => {
    try {
        ...
        const { data } = await API.patch("/user", userInfo);
        if (data.message) {
            toast.success(data.message);
        } else {
            toast.error(data.error);
