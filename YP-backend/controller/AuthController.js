...
export const changePassword = async (req, res) => {
    try {
        ...
        if (current_password === new_password) {
            return res.send({ error: "Old Password and New Password cannot be the same" });
        }

        if (new_password !== confirm_password) {
            return res.send({ error: "New Password and Confirm Password must be same" });
        }
...
export const postResetToken = async (req, res) => {
    try {
        ...
        if (!user) {
            return res.send({ error: 'Invalid or expired token. Please request a new password reset.' });
        }
...
export const verifyEmail = async (req, res) => {
    try {
        ...
        sendEmailVerification(email, token, res);
    } catch (error) {
        console.error(error.message);
        res.send({ error: "Internal Server Error" });
    }
