...
export const getSeoById = async (req, res) => {
    try {
        ...
        const seo = await Seo.findOne({ uniqueId: uniqueId });
        return res.status(200).json(seo);
    } catch (error) {
        console.log(error.message)
        return res.send({ error: "Internal server error!" })
...
export const addSeo = async (req, res) => {
    try {
        const { title, slug, meta_tags, description, primary_focus_keyword, json_schema, course_id, course_name, property_id, property_name } = req.body;
        ...
        const existSeo = await Seo.findOne({ title: title, property_id: property_id });
        if (!existSeo) {
...
export const updateSeo = async (req, res) => {
    try {
        const uniqueId = req.params.uniqueId;
        const { title, slug, meta_tags, description, primary_focus_keyword, json_schema, status } = req.body;
        await Seo.findOneAndUpdate({ uniqueId: uniqueId }, {
            $set: {
                title,
                slug,
                meta_tags,
...
export const deleteSeo = async (req, res) => {
    try {
        ...
        const seo = await Seo.findOne({ uniqueId: uniqueId });
        if (seo) {
            await Seo.findOneAndDelete({ uniqueId: uniqueId })
                .then(result => {
                    console.log(result)
