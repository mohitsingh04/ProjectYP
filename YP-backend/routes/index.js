...
import { changePassword, forgotPassword, getEmailVerification, getResetToken, login, postResetToken, profile, register, verifyEmail } from "../controller/AuthController.js";
import { Authentication, userData } from "../middleware/index.js";
import { deleteUser, deleteUserProfile, getUser, getUserById, updateUser } from "../controller/UserController.js";
...
import { addProperty, deleteProperty, getProperty, getPropertyById, getPropertyBySlug, updateProperty, updatePropertyImages } from "../controller/PropertyController.js";
import { addTeacher, deleteTeacher, getTeacher, getTeacherById, updateTeacher } from "../controller/TeachersController.js";
import { addReview, deleteReview, getReview, getReviewById, updateReview } from "../controller/ReviewsController.js";
import { addFaq, deleteFaq, getFaq, getFaqById, updateFaq } from "../controller/FaqsController.js";
...
import { addSeo, deleteSeo, getSeo, getSeoById, updateSeo } from "../controller/SeoController.js";
import { addSearch, getSearch } from "../controller/SearchController.js";
import { addPropertyCourse, getPropertyCourse, getPropertyCourseById, updatePropertyCourse } from "../controller/PropertyCourseController.js";
import { addBusinessHours, getBusinessHours } from "../controller/BusinessHourController.js";
...
// Category Route
const categoryUpload = upload.fields([{ name: 'category_icon', maxCount: 1 }, { name: 'featured_image', maxCount: 1 }])
router.get("/category", Authentication, getCategory);
router.post("/category", Authentication, categoryUpload, addCategory);
router.patch("/category/:uniqueId", Authentication, categoryUpload, updateCategory);
router.delete("/category/:uniqueId", Authentication, deleteCategory);
router.get("/category/:uniqueId", Authentication, getCategoryById);

// Property Route
const propertyUpload = upload.fields([{ name: 'property_icon', maxCount: 1 }, { name: 'featured_image', maxCount: 1 }])
router.get("/property", Authentication, getProperty);
router.post("/property", Authentication, propertyUpload, addProperty);
router.patch("/property/:uniqueId", Authentication, propertyUpload, updateProperty);
router.patch("/property/images/:uniqueId", Authentication, propertyUpload, updatePropertyImages);
...
router.patch("/teacher/:uniqueId", Authentication, teacherProfile, updateTeacher);
...
router.get("/gallery/:uniqueId", Authentication, getGalleryById);
...
router.post("/property-course", Authentication, courseUpload, addPropertyCourse);
router.patch("/property-course/:uniqueId", Authentication, courseUpload, updatePropertyCourse);
